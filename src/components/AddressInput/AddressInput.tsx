    ...
    <div className={styles.container}>
      ...
      <div className="relative">
        <input
          ...
          readOnly={readOnly}
        />
        {(value || (savedValues.length > 0 && !readOnly)) && (
          <button
            onClick={() => clearIcon ? onChange("") : setIsDropdownOpen(!isDropdownOpen)}
            className={styles.clearButton}
            type="button"
          >
            {clearIcon ? <X size={16} /> : <ChevronDown size={16} />}
          </button>
        )}
        ...
        {isDropdownOpen && savedValues.length > 0 && (
          <div 
            className="absolute z-10 w-full bg-white border border-gray-200 rounded-lg shadow-lg"
            onMouseLeave={() => setIsDropdownOpen(false)}
          >
            {isLoading ? (
              <div className="px-4 py-2 text-sm text-gray-500">Loading...</div>
            ) : poolCoins? ( poolCoins.map((item, index) => (
              <button
                  key={index}
